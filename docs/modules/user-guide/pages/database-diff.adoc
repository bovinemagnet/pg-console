...
[cols="1,2,2"]
|===
|Preset |Description |Excluded Patterns

|*No filters*
|Include all objects without any filtering
|None

|*Exclude temp/backup tables*
|Excludes temporary and backup tables commonly used during development
|`temp_*`, `tmp_*`, `*_backup`, `*_bak`, `zz_*`

|*Exclude system schemas*
|Excludes PostgreSQL system schemas and pg-console internal schema
|`pg_catalog`, `information_schema`, `pgconsole`

|*Production-safe defaults*
|Combines temp/backup exclusion with system schema exclusion for clean production comparisons
|All patterns from both presets above
|===

==== When to Use Each Preset

*No filters*::
Use when you need a complete comparison of all objects, including temporary tables and system schemas. Suitable for comprehensive audits.

*Exclude temp/backup tables*::
Use when comparing schemas that may contain development artefacts like temporary tables or backup copies. This focuses the comparison on production-relevant objects.

*Exclude system schemas*::
Use when you want to compare only user-defined objects, excluding PostgreSQL system catalogues and pg-console's internal schema.

*Production-safe defaults*::
Recommended for most production comparisons. Excludes both temporary/backup tables and system schemas, providing a clean comparison of actual application objects.

=== Custom Exclude Patterns

In addition to presets, you can specify custom exclusion patterns using wildcard syntax:

[source,text]
----
audit_*, log_*, *_archive, staging_*
----
...
* `zz_*` - Excludes tables prefixed with `zz_` (common for deprecated objects)

Enter patterns as a comma-separated list in the *Exclude Patterns* field.
