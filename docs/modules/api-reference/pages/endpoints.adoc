...
* <<diagnostics-history-api,Diagnostics History API>> - Persisted historical metrics queries
* <<stopwatch-api,Stopwatch API>> - Point-in-time monitoring sessions
* <<window-comparison-api,Window Comparison API>> - Two-window metric comparison
...
[#metrics-history-api]
=== GET /api/diagnostics/metrics-history/connections

Returns historical database connection metrics for the specified time window.
...
|`minutes`
|Query (optional)
|Time window in minutes (default: `30`, max: `1440` for 24 hours)
|===
...
----
{
  "timestamps": [1771283819008, 1771283824000, 1771283829000],
  "active": [0.0, 1.0, 0.0],
  "idle": [12.0, 12.0, 12.0],
  "idleInTransaction": [0.0, 0.0, 0.0],
  "dataPoints": 3
}
----
...
[source,bash]
----
curl http://localhost:8080/api/diagnostics/metrics-history/connections?minutes=30
----

'''

=== GET /api/diagnostics/metrics-history/transactions

Returns historical transaction rate metrics (commits and rollbacks per second).
...
|`minutes`
|Query (optional)
|Time window in minutes (default: `30`, max: `1440`)
|===
...
----
{
  "timestamps": [1771283824000, 1771283829000],
  "commitsRate": [11.6, 1.4],
  "rollbacksRate": [0.4, 0.0],
  "dataPoints": 2
}
----

*Note:* Rates are calculated server-side using delta/seconds from cumulative counters. First data point is omitted (requires previous value for rate calculation).

==== Example

[source,bash]
----
curl http://localhost:8080/api/diagnostics/metrics-history/transactions?minutes=60
----

'''

=== GET /api/diagnostics/metrics-history/tuples

Returns historical tuple operation rates (inserts, updates, deletes per second).
...
*Note:* Rates are calculated server-side. First data point is omitted.

==== Example

[source,bash]
----
curl http://localhost:8080/api/diagnostics/metrics-history/tuples?minutes=30
----

'''

=== GET /api/diagnostics/metrics-history/cache

Returns historical cache hit ratio metrics as percentages (0-100).
...
----
{
  "timestamps": [1771283819008, 1771283824000],
  "bufferHitRatio": [99.13, 99.13],
  "indexHitRatio": [100.0, 100.0],
  "dataPoints": 2
}
----
...
[source,bash]
----
curl http://localhost:8080/api/diagnostics/metrics-history/cache?minutes=10
----

'''

=== GET /api/diagnostics/metrics-history/export

Exports all historical metrics for the specified time window in JSON or CSV format.
...
Returns data in comma-separated format with header row:
...
*Note:* CSV export includes cumulative counter values (not rates). Calculate rates in post-processing if needed.
...
[source,bash]
----
curl http://localhost:8080/api/diagnostics/metrics-history/export?format=json&minutes=60
----
...
[source,bash]
----
curl http://localhost:8080/api/diagnostics/metrics-history/export?format=csv&minutes=30 > metrics.csv
----
...
Endpoints for querying persisted historical metrics from database tables, providing up to 7 days of data.

==== List Tracked Queries

[source,http]
----
GET /api/diagnostics/history/queries/list?instance={instance}&hours={hours}
----

Returns a list of tracked queries from persisted `query_metrics_history` data.
...
*Response:* JSON array of query summaries with `query_id`, `query_text`, `total_calls`, `mean_time_ms`.

==== Example

[source,bash]
----
curl http://localhost:8080/api/diagnostics/history/queries/list?instance=default&hours=24
----
...
[source,http]
----
GET /api/diagnostics/history/queries?instance={instance}&queryId={queryId}&hours={hours}
----

Returns time-series performance data for a specific query.
...
|`queryId`
|String
|Query hash identifier from `pg_stat_statements` (required)
...
*Response:* JSON with `timestamps` and series arrays for `mean_time`, `total_calls`, `total_time`.

==== Example

[source,bash]
----
curl http://localhost:8080/api/diagnostics/history/queries?instance=default&queryId=abc123&hours=48
----
...
[source,http]
----
GET /api/diagnostics/history/database?instance={instance}&database={database}&hours={hours}
----

Returns time-series metrics for a specific database.
...
*Response:* JSON with `timestamps` and series arrays for `num_backends`, `xact_commit`, `xact_rollback`, `cache_hit_ratio`, `tup_inserted`, `tup_updated`, `tup_deleted`.

==== Example

[source,bash]
----
curl http://localhost:8080/api/diagnostics/history/database?instance=default&database=myapp&hours=24
----
...
[source,http]
----
GET /api/diagnostics/history/infrastructure?instance={instance}&hours={hours}
----

Returns time-series infrastructure metrics (WAL, checkpoints, buffers).
...
*Response:* JSON with `timestamps` and series arrays for infrastructure metrics.

==== Example

[source,bash]
----
curl http://localhost:8080/api/diagnostics/history/infrastructure?instance=default&hours=72
----
...
Endpoints for managing stopwatch monitoring sessions. Only one session can be active per instance at a time.

==== Start Session

[source,http]
----
POST /api/stopwatch/start?instance={instance}
----

Starts a new stopwatch session, capturing a point-in-time metric snapshot.
...
*Response:* `200 OK` with JSON session object including `id`, `startedAt`, `status`.

*Error:* `409 Conflict` if a session is already running for this instance.

==== Example

[source,bash]
----
curl -X POST http://localhost:8080/api/stopwatch/start?instance=default
----
...
[source,http]
----
POST /api/stopwatch/stop?instance={instance}
----

Stops the active session, capturing an end snapshot and computing deltas.
...
*Response:* `200 OK` with JSON session object including `id`, `startedAt`, `stoppedAt`, `elapsedDisplay`.

*Error:* `404 Not Found` if no active session exists.

==== Example

[source,bash]
----
curl -X POST http://localhost:8080/api/stopwatch/stop?instance=default
----
...
[source,http]
----
POST /api/stopwatch/cancel?instance={instance}
----

Cancels the active session without saving end snapshots.
...
[source,bash]
----
curl -X POST http://localhost:8080/api/stopwatch/cancel?instance=default
----
...
[source,http]
----
GET /api/stopwatch/active?instance={instance}
----
...
*Response:* `200 OK` with JSON session object, or `204 No Content` if no active session exists.

==== Example

[source,bash]
----
curl http://localhost:8080/api/stopwatch/active?instance=default
----
...
[source,http]
----
GET /api/stopwatch/recent?instance={instance}
----

Returns recent completed and cancelled sessions.
...
*Response:* `200 OK` with JSON array of session objects.

==== Example

[source,bash]
----
curl http://localhost:8080/api/stopwatch/recent?instance=default
----
...
[source,http]
----
GET /api/stopwatch/status?instance={instance}
----

Returns live status of the active session (used by htmx polling).
...
*Response:* `200 OK` with JSON status object including elapsed time and current metrics.

==== Example

[source,bash]
----
curl http://localhost:8080/api/stopwatch/status?instance=default
----

'''

[#window-comparison-api]
=== Window Comparison API

Endpoints for comparing metrics between two time windows.

==== Get Presets

[source,http]
----
GET /api/window-compare/presets?instance={instance}
----

Returns available comparison presets with availability status.
...
*Response:* `200 OK` with JSON array of preset objects, each containing `name`, `startA`, `endA`, `startB`, `endB`, `available` (boolean).

==== Example

[source,bash]
----
curl http://localhost:8080/api/window-compare/presets?instance=default
----
...
[source,http]
----
GET /api/window-compare/compare?instance={instance}&startA={iso8601}&endA={iso8601}&startB={iso8601}&endB={iso8601}
----

Executes a comparison between two time windows.
...
|`startA`
|String
|Start of Window A in ISO-8601 format (required)

|`endA`
|String
|End of Window A in ISO-8601 format (required)

|`startB`
|String
|Start of Window B in ISO-8601 format (required)

|`endB`
|String
|End of Window B in ISO-8601 format (required)
|===

*Response:* `200 OK` with JSON comparison result including:

* `windowA` / `windowB` - Window metadata (`start`, `end`, `duration`, `sampleCount`)
* `deltas` - Array of metric deltas (`name`, `startValue`, `endValue`, `delta`, `deltaPercent`, `direction`, `cssClass`)
* `queryChanges` - Array of per-query changes (`queryText`, `meanTimeA`, `meanTimeB`, `meanTimeDelta`, `callCountA`, `callCountB`, `status`, `regression`)
* `overallDirection` - `improved`, `degraded`, or `mixed`
* `hasData` - Boolean indicating sufficient data

==== Example

[source,bash]
----
curl "http://localhost:8080/api/window-compare/compare?instance=default&startA=2026-03-04T08:00:00Z&endA=2026-03-04T09:00:00Z&startB=2026-03-04T10:00:00Z&endB=2026-03-04T11:00:00Z"
----
