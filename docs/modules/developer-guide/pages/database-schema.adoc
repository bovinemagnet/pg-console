...
* Stopwatch sessions for before/after metric comparison
...
These tables support the Stopwatch feature for recording before/after metric snapshots during monitoring sessions.

=== stopwatch_session

Stores stopwatch monitoring sessions with point-in-time metric snapshots captured at start and stop. Used by the Stopwatch feature to show before/after comparison of database metrics.
...
|`status`
|`VARCHAR(20)`
|Session status: 'running', 'stopped', or 'cancelled'
...
|`stopped_at`
|`TIMESTAMP WITH TIME ZONE`
|When the session was stopped (NULL if running or cancelled)
...
|`start_snapshot`
|`JSONB`
|Point-in-time metric snapshot at session start (connections, queries, cache ratio, transactions, tuples, locks)

|`end_snapshot`
|`JSONB`
|Point-in-time metric snapshot at session stop

|`top_queries_start`
|`JSONB`
|Top 20 queries by total time at session start (serialised SlowQuery list)

|`top_queries_end`
|`JSONB`
|Top 20 queries by total time at session stop
...
`idx_stopwatch_session_instance_started`::
B-tree index on `(instance_id, started_at DESC)` for retrieving recent sessions per instance.

`idx_stopwatch_session_status`::
B-tree index on `(status)` for finding active sessions.

==== JSONB Snapshot Structure

The `start_snapshot` and `end_snapshot` columns store metric snapshots as JSONB with the following structure:
...
SELECT id,
       start_snapshot->>'totalConnections' AS start_conns,
       end_snapshot->>'totalConnections' AS end_conns,
       (end_snapshot->>'cacheHitRatio')::numeric - (start_snapshot->>'cacheHitRatio')::numeric AS cache_delta
FROM pgconsole.stopwatch_session
WHERE id = 1;
----
...
`V2__stopwatch_sessions.sql`::
Creates the `pgconsole.stopwatch_session` table for storing stopwatch monitoring sessions with JSONB metric snapshots. Includes indexes for instance/time lookups and status filtering.
...
-- Delete old stopwatch sessions
DELETE FROM pgconsole.stopwatch_session
...
VACUUM pgconsole.stopwatch_session;
----
