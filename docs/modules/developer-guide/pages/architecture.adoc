...
`LiveChartHistoryStore`::
Application-scoped in-memory store that samples database metrics (connections, transactions, tuples, cache) every 5 seconds via `PostgresService` and retains up to 24 hours of data points. Provides time-window-filtered history for the Metrics History dashboard. Server-side rate calculation transforms cumulative counters into per-second rates.
