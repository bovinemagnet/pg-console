...
.pg-console overview dashboard
image::screenshot-overview-dark.png[pg-console Overview Dashboard,width=800]
...
* *Historical Trends* - Query, database, and infrastructure trend analysis from persisted metrics (up to 7 days)
* *Stopwatch* - Start/stop monitoring sessions to measure the impact of database operations
* *Window Comparison* - Side-by-side comparison of two time windows to identify trends and regressions
...
* *Metrics History* - Interactive Chart.js dashboard with up to 7 days of historical metrics from persisted storage and JSON/CSV export
...
|Query performance trends from persisted history (up to 7 days)

|`/diagnostics/database-trends`
|Per-database trend charts from persisted history

|`/diagnostics/infrastructure-trends`
|Infrastructure trend charts (WAL, checkpoints, buffers)

|`/stopwatch`
|Start/stop monitoring sessions with before/after comparison

|`/window-compare`
|Side-by-side comparison of two time windows
...
* xref:user-guide:stopwatch.adoc[Stopwatch] - Start/stop monitoring sessions with before/after comparison
* xref:user-guide:window-compare.adoc[Window Comparison] - Side-by-side time window comparison
