...
public class EmailNotificationSender extends AbstractNotificationSender {
	...
	public NotificationResult send(NotificationChannel channel, ActiveAlert alert) {
		...
		try {
			...
			Mail mail = Mail.withHtml(String.join(",", config.getRecipients()), subject, htmlBody).setText(textBody);
			...
			return NotificationResult.success(channel.getId(), channel.getName(), channel.getChannelType(), alert.getAlertId()).withAlertDetails(alert.getAlertType(), alert.getAlertSeverity(), alert.getAlertMessage(), alert.getInstanceName());
		} catch (Exception e) {
	...
	public NotificationResult sendTest(NotificationChannel channel) {
		...
		try {
			String subject = (config.getSubjectPrefix() != null ? config.getSubjectPrefix() + " " : "") + "Test Notification - PG Console";
			...
			return NotificationResult.success(channel.getId(), channel.getName(), channel.getChannelType(), "test");
		} catch (Exception e) {
	...
	private String buildSubject(EmailConfig config, ActiveAlert alert) {
		...
		subject.append("[").append(alert.getAlertSeverity()).append("] ").append(alert.getAlertType());
