...
public class DiscordNotificationSender extends AbstractNotificationSender {
	...
	public boolean validateConfig(NotificationChannel channel) {
		...
		return webhookUrl != null && !webhookUrl.isBlank() && webhookUrl.contains("discord.com/api/webhooks/");
	}
	...
	private String buildEmbedPayload(NotificationChannel channel, DiscordConfig config, ActiveAlert alert) {
		...
		// Title
		payload.append("\"title\":\"").append(getSeverityEmoji(alert.getAlertSeverity())).append(" ").append(escapeJson(alert.getAlertType())).append("\",");
	...
	private String buildTestPayload(NotificationChannel channel, DiscordConfig config) {
		...
		if (config.isUseEmbeds()) {
			...
			payload.append("\"content\":\"✅ Test notification from PG Console - Channel '").append(escapeJson(channel.getName())).append("' is configured correctly!\"");
		}
