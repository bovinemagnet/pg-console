...
[abstract]
This guide covers the advanced diagnostics features in pg-console, including pipeline risk monitoring, TOAST bloat analysis, index redundancy detection, statistical freshness tracking, write/read ratio analysis, HOT update efficiency, column correlation, live charts, metrics history, and transaction ID wraparound monitoring.
...
* Historical metric analysis (metrics history)
* Historical trend analysis (query, database, infrastructure metrics over up to 7 days)
* Time-based comparison tools (stopwatch sessions, window comparison)
...
.Diagnostics index page in dark mode
image::screenshot-diagnostics-index-dark.png[Diagnostics Index,width=800]

The diagnostics index page (`/diagnostics`) provides a central overview of all diagnostic tools with summary counts and quick navigation.

=== Accessing the Index

Navigate to `/diagnostics` or click on the Diagnostics section header in the navigation menu.

=== Summary Dashboard

The index page displays cards for each diagnostic tool, organised into three categories:

==== Performance Diagnostics

* *Pipeline Risk* - Count of queue/pipeline tables with processing backlogs
* *Statistical Freshness* - Count of tables with stale statistics
* *Write/Read Ratio* - Count of write-heavy tables
* *HOT Efficiency* - Count of tables with low HOT update efficiency

==== Storage & Indexes

* *TOAST Bloat* - Count of tables with significant TOAST bloat
* *Index Redundancy* - Count of redundant or overlapping indexes
* *Correlation* - Count of columns where CLUSTER is recommended

==== Health Monitoring

* *XID Wraparound* - Count of databases at risk of transaction ID wraparound
* *Live Charts* - Link to real-time metric visualisation
* *Metrics History* - Historical metric trends with configurable time windows
* *Query Trends* - Historical query performance analysis from persisted data
* *Database Trends* - Per-database metric trends over time
* *Infrastructure Trends* - WAL, checkpoint, and buffer activity trends

=== Issue Indicators

Each card displays a colour-coded badge showing the count of issues:
...
|Yellow (`bg-warning`)
|1-5 issues detected - review recommended

|Red (`bg-danger`)
|More than 5 issues or critical status - attention required
|===

When issues are detected, a warning banner appears at the top of the page showing the total count across all diagnostics.

=== Quick Navigation

Each diagnostic card includes a "View Details" link that navigates directly to the full diagnostic page for detailed analysis and recommendations.

=== Use Cases

The diagnostics index page is useful for:

* *Daily health checks* - Quick overview of database health status
* *Triage* - Identify which areas need attention first
* *Dashboarding* - Entry point to the diagnostics subsystem
* *Monitoring integration* - Central location for health status
...
.Live charts dashboard in dark mode
image::screenshot-live-charts-dark.png[Live Charts Dashboard,width=800]
...
The Metrics History dashboard (`/diagnostics/metrics-history`) provides historical visualisation of database metrics with configurable time windows and export capabilities.

=== What It Does

Unlike Live Charts which show only real-time data that is lost on page navigation, Metrics History:

* Retains up to 24 hours of metrics in-memory
* Captures data every 5 seconds since application start
* Displays 4 Chart.js charts in a 2×2 grid
* Supports flexible time windows (5 min to 24 hours)
...
|Active, Idle, and Idle in Transaction connection counts from `pg_stat_activity`

|Transaction Rate
|Commits per second and rollbacks per second (server-side rate calculation)

|Tuple Operations Rate
|Inserts, updates, and deletes per second

|Cache Hit Ratio
|Buffer cache % and index cache % (0-100 scale)
|===

=== Time Windows

Select preset time ranges using dashboard controls:

* *5 minutes* - Recent activity
* *10 minutes* - Short-term trends
* *30 minutes* - Default view
* *1 hour* - Medium-term analysis
* *24 hours* - Full retention period

Time windows are URL-based: `/diagnostics/metrics-history?minutes=30`
...
Exports include all data points within the selected time window.
...
[cols="1,2,2"]
|===
|Aspect |Metrics History |Live Charts
...
For complete documentation, see xref:metrics-history.adoc[Metrics History].

[[xid-wraparound]]
== Transaction ID Wraparound Monitoring

.XID wraparound monitoring dashboard in dark mode
image::screenshot-xid-wraparound-dark.png[XID Wraparound Dashboard,width=800]
...
.Query trends dashboard in dark mode
image::screenshot-query-trends-dark.png[Query Trends Dashboard,width=800]

The Query Trends dashboard (`/diagnostics/query-trends`) provides historical analysis of query performance from persisted `query_metrics_history` data, covering up to 7 days.

=== Accessing Query Trends

Navigate to *Diagnostics* → *Query Trends* or visit `/diagnostics/query-trends`.

=== Features

* *Query list* - Table of tracked queries with sparklines showing performance trends
* *Detailed charts* - Click a query to expand Chart.js charts of mean execution time, call count, and total time over the selected period
* *Time range selector* - Choose from 1 hour to 7 days of historical data
* *Regression detection* - Queries with significant performance degradation are highlighted

=== Data Source

Query Trends reads from the `pgconsole.query_metrics_history` table, which is populated by the background metrics sampler every 60 seconds. This provides much longer retention than the in-memory Metrics History (up to 7 days vs 24 hours).

=== Use Cases

* *Post-deployment analysis* - Verify query performance hasn't degraded after releases
* *Capacity planning* - Identify queries with growing execution times
* *Optimisation targeting* - Find the queries consuming the most total database time

=== Configuration

Enable or disable via the feature toggle:

[source,bash]
----
export PG_CONSOLE_DASH_QUERY_TRENDS=true   # default: true
----

Requires history sampling to be enabled (`PG_CONSOLE_HISTORY_ENABLED=true`).

[[database-trends]]
== Database Trends

.Database trends dashboard in dark mode
image::screenshot-database-trends-dark.png[Database Trends Dashboard,width=800]

The Database Trends dashboard (`/diagnostics/database-trends`) shows per-database metrics over time from persisted `database_metrics_history` data.

=== Accessing Database Trends

Navigate to *Diagnostics* → *Database Trends* or visit `/diagnostics/database-trends`.

=== Features

* *Database selector* - Choose which database to analyse
* *Connection charts* - Backend connection counts over time
* *Transaction rate charts* - Commit and rollback rates per second
* *Cache ratio charts* - Buffer cache and index cache hit ratios
* *Tuple operation charts* - Insert, update, and delete rates
* *Time range selector* - Choose from 1 hour to 7 days

=== Data Source

Reads from the `pgconsole.database_metrics_history` table. Cumulative counters (transactions, tuples) are converted to per-second rates using delta calculations between adjacent samples.

=== Use Cases

* *Per-database capacity planning* - Identify databases with growing workloads
* *Cross-database comparison* - Compare activity patterns across databases
* *Deadlock trend analysis* - Track deadlock frequency over time

=== Configuration

[source,bash]
----
export PG_CONSOLE_DASH_DATABASE_TRENDS=true   # default: true
----

[[infrastructure-trends]]
== Infrastructure Trends

.Infrastructure trends dashboard in dark mode
image::screenshot-infrastructure-trends-dark.png[Infrastructure Trends Dashboard,width=800]

The Infrastructure Trends dashboard (`/diagnostics/infrastructure-trends`) shows WAL, checkpoint, and buffer activity over time from persisted data.

=== Accessing Infrastructure Trends

Navigate to *Diagnostics* → *Infrastructure Trends* or visit `/diagnostics/infrastructure-trends`.

=== Features

* *WAL activity charts* - WAL generation rates over time
* *Checkpoint charts* - Checkpoint frequency and timing
* *Buffer activity charts* - Buffer allocation and write patterns
* *Time range selector* - Choose from 1 hour to 7 days

=== Data Source

Reads from the `pgconsole.infrastructure_metrics_history` table (if available) or derives infrastructure data from `system_metrics_history`.

=== Use Cases

* *Storage capacity planning* - Track WAL generation growth
* *Checkpoint tuning* - Identify if checkpoints are too frequent or too slow
* *I/O pattern analysis* - Understand buffer write patterns over time

=== Configuration

[source,bash]
----
export PG_CONSOLE_DASH_INFRASTRUCTURE_TRENDS=true   # default: true
----
