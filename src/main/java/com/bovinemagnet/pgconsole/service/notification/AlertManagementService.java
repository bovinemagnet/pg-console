...
public class AlertManagementService {
     ...
     * @return the created alert
     */
    public ActiveAlert fireAlert(String alertType, String severity, String message,
                                  String instanceName, Long escalationPolicyId) {
    ...
    public void acknowledgeAlert(Long alertId, String acknowledgedBy) {
        alertRepository.findById(alertId).ifPresent(alert -> {
            if (!alert.isAcknowledged()) {
                ...
                LOG.infof("Alert %s acknowledged by %s", alert.getAlertId(),
    ...
    public List<NotificationResult> resolveAlert(Long alertId, String resolvedBy, boolean sendResolution) {
        ...
        if (!alert.isResolved()) {
            ...
            LOG.infof("Alert %s resolved by %s", alert.getAlertId(),
    ...
    public AlertSilence createSilence(AlertSilence silence) {
        ...
        LOG.infof("Created alert silence: %s until %s", silence.getName(),
            silence.getEndTimeFormatted());
        return saved;
    }
     ...
     */
    public AlertSilence createQuickSilence(String alertType, String instanceName,
                                            int durationMinutes, String createdBy) {
        AlertSilence silence = new AlertSilence(
            "Quick silence: " + alertType,
            Instant.now().plus(Duration.ofMinutes(durationMinutes))
        );
        silence.setCreatedBy(createdBy);

        if (alertType != null) {
            silence.addMatcher(new AlertSilence.Matcher(
                "alertType", AlertSilence.Matcher.Operator.EQUALS, alertType
            ));
        }

        if (instanceName != null) {
            silence.addMatcher(new AlertSilence.Matcher(
                "instanceName", AlertSilence.Matcher.Operator.EQUALS, instanceName
            ));
        }
    ...
    public MaintenanceWindow createMaintenanceWindow(MaintenanceWindow window) {
        ...
        LOG.infof("Created maintenance window: %s from %s to %s",
            window.getName(), window.getStartTimeFormatted(), window.getEndTimeFormatted());
        return saved;
    }
    ...
    public AlertStats getAlertStats() {
        ...
        int unacknowledgedCount = (int) active.stream()
            .filter(a -> !a.isAcknowledged())
            .count();

        int resolvedLast24h = (int) recent.stream()
            .filter(a -> a.isResolved() && a.getResolvedAt() != null &&
                a.getResolvedAt().isAfter(Instant.now().minus(Duration.ofHours(24))))
            .count();
    ...
    public void cleanupOldData() {
        ...
        LOG.infof("Cleanup completed: %d old alerts, %d expired silences deleted",
            deletedAlerts, deletedSilences);
    }
