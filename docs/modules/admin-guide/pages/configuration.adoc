...
export PG_CONSOLE_DATABASES=myapp_prod
----
...
|`PG_CONSOLE_METADATA_DATASOURCE`
|(empty)
|Datasource for pgconsole metadata. Options: empty (use default), `metadata` (dedicated datasource), or an instance name

|`PG_CONSOLE_METADATA_URL`
|(none)
|JDBC URL for dedicated metadata database (when using `metadata` datasource)

|`PG_CONSOLE_METADATA_USER`
|(none)
|Username for dedicated metadata database

|`PG_CONSOLE_METADATA_PASSWORD`
|(none)
|Password for dedicated metadata database
|===

=== Option 1: Default (Backwards Compatible)

Leave `PG_CONSOLE_METADATA_DATASOURCE` unset. Metadata is stored in the monitored database (original behaviour).

[source,bash]
----
# No configuration needed - metadata stored in default monitored database
export POSTGRES_URL=jdbc:postgresql://db:5432/postgres
export POSTGRES_USER=postgres
export POSTGRES_PASSWORD=postgres
----
...
Use a separate database for storing all pg-console metadata. This is the recommended approach for production monitoring.

[source,bash]
----
# Configure monitored instance (can be read-only user)
...
[source,properties]
----
# Monitored database (read-only access)
quarkus.datasource.db-kind=postgresql
quarkus.datasource.jdbc.url=${POSTGRES_URL:jdbc:postgresql://prod-db:5432/postgres}
quarkus.datasource.username=${POSTGRES_USER:readonly_monitor}
quarkus.datasource.password=${POSTGRES_PASSWORD:secure_password}

# Metadata datasource selection
pg-console.metadata.datasource=metadata

# Dedicated metadata datasource
quarkus.datasource.metadata.db-kind=postgresql
quarkus.datasource.metadata.jdbc.url=${PG_CONSOLE_METADATA_URL:jdbc:postgresql://control-db:5432/pgconsole}
quarkus.datasource.metadata.username=${PG_CONSOLE_METADATA_USER:pgconsole_admin}
quarkus.datasource.metadata.password=${PG_CONSOLE_METADATA_PASSWORD:metadata_password}
----

=== Option 3: Use Another Instance's Datasource

In multi-instance deployments, you can store metadata in one of the configured instance datasources:
...
This approach is useful when you have a non-production instance that can host the metadata schema.

=== Initialising the Schema

When using a dedicated metadata database, the schema must be initialised. Use the CLI command:
...
java -jar pg-console.jar init-schema --metadata --force
----

For more details on CLI commands, see xref:cli-reference.adoc[CLI Reference].

=== Use Cases

==== Read-Only Production Monitoring

Monitor a production database without write permissions:

[source,bash]
----
# Production database (read-only user, no schema creation)
export POSTGRES_URL=jdbc:postgresql://prod-db:5432/postgres
export POSTGRES_USER=monitoring_readonly
export POSTGRES_PASSWORD=readonly_password
export PG_CONSOLE_SCHEMA_ENABLED=false

# All metadata stored in separate control database
export PG_CONSOLE_METADATA_DATASOURCE=metadata
export PG_CONSOLE_METADATA_URL=jdbc:postgresql://control-db:5432/pgconsole
export PG_CONSOLE_METADATA_USER=pgconsole_admin
export PG_CONSOLE_METADATA_PASSWORD=control_password
----

==== Centralised Metadata for Multi-Instance

Store metadata from all monitored instances in a single database:

[source,bash]
----
export PG_CONSOLE_INSTANCES=prod_us,prod_eu,staging

# All instances share metadata storage
export PG_CONSOLE_METADATA_DATASOURCE=metadata
export PG_CONSOLE_METADATA_URL=jdbc:postgresql://central-db:5432/pgconsole_metadata
export PG_CONSOLE_METADATA_USER=pgconsole
export PG_CONSOLE_METADATA_PASSWORD=metadata_password
----

This enables cross-instance queries and consolidated reporting.
...
export PG_CONSOLE_DASH_METRICS_HISTORY=true         # Historical metrics with Chart.js charts
export PG_CONSOLE_DASH_QUERY_TRENDS=true            # Historical query performance trends
export PG_CONSOLE_DASH_DATABASE_TRENDS=true         # Per-database metric trends
export PG_CONSOLE_DASH_INFRASTRUCTURE_TRENDS=true   # WAL, checkpoint, and buffer trends
export PG_CONSOLE_DASH_STOPWATCH=true               # Start/stop monitoring sessions with before/after comparison
export PG_CONSOLE_DASH_WINDOW_COMPARE=true          # Side-by-side comparison of two time windows
----
...
pg-console.dashboards.diagnostics.metrics-history=${PG_CONSOLE_DASH_METRICS_HISTORY:true}
pg-console.dashboards.diagnostics.query-trends=${PG_CONSOLE_DASH_QUERY_TRENDS:true}
pg-console.dashboards.diagnostics.database-trends=${PG_CONSOLE_DASH_DATABASE_TRENDS:true}
pg-console.dashboards.diagnostics.infrastructure-trends=${PG_CONSOLE_DASH_INFRASTRUCTURE_TRENDS:true}
pg-console.dashboards.diagnostics.stopwatch=${PG_CONSOLE_DASH_STOPWATCH:true}
pg-console.dashboards.diagnostics.window-compare=${PG_CONSOLE_DASH_WINDOW_COMPARE:true}
----

==== History-Dependent Diagnostic Pages

NOTE: The Query Trends, Database Trends, Infrastructure Trends, and Window Compare pages require history sampling to be active in order to display meaningful data. Ensure `PG_CONSOLE_HISTORY_ENABLED=true` (the default) before enabling these toggles.

[cols="2,1,3"]
|===
|Environment Variable |Default |Description

|`PG_CONSOLE_DASH_QUERY_TRENDS`
|`true`
|Enable or disable the Query Trends page (`/diagnostics/query-trends`). Shows historical query performance from persisted data.

|`PG_CONSOLE_DASH_DATABASE_TRENDS`
|`true`
|Enable or disable the Database Trends page (`/diagnostics/database-trends`). Shows per-database metric trends.

|`PG_CONSOLE_DASH_INFRASTRUCTURE_TRENDS`
|`true`
|Enable or disable the Infrastructure Trends page (`/diagnostics/infrastructure-trends`). Shows WAL, checkpoint, and buffer trends.

|`PG_CONSOLE_DASH_STOPWATCH`
|`true`
|Enable or disable the Stopwatch page (`/stopwatch`). Allows start/stop monitoring sessions with before/after metric comparison.

|`PG_CONSOLE_DASH_WINDOW_COMPARE`
|`true`
|Enable or disable the Window Comparison page (`/window-compare`). Allows side-by-side comparison of two time windows.
|===
