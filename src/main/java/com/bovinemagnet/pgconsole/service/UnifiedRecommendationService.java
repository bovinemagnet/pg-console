...
public class UnifiedRecommendationService {
	 ...
	 */
	public List<UnifiedRecommendation> getRecommendationsBySource(String instanceName, UnifiedRecommendation.Source source) {
		return getRecommendations(instanceName)
			.stream()
	 ...
	 * @return filtered list of recommendations
	 */
	public List<UnifiedRecommendation> getRecommendationsBySeverity(String instanceName, UnifiedRecommendation.Severity severity) {
		return getRecommendations(instanceName)
			.stream()
	...
	public List<UnifiedRecommendation> getTopRecommendations(String instanceName, int limit) {
		return getRecommendations(instanceName).stream().limit(limit).collect(Collectors.toList());
	}
	...
	private UnifiedRecommendation convertIndexRecommendation(String instanceName, IndexRecommendation ir) {
		...
		ur.setPriorityScore(UnifiedRecommendation.calculatePriorityScore(ur.getSeverity(), ur.getEstimatedImpact(), ur.getEstimatedEffort()));
	...
	private UnifiedRecommendation convertQueryRegression(String instanceName, QueryRegression qr) {
		...
		ur.setRationale(String.format("Mean time increased from %s to %s (%s calls)", qr.getPreviousMeanTimeFormatted(), formatDuration(qr.getCurrentMeanTime()), qr.getCurrentCalls()));
	...
	private List<UnifiedRecommendation> checkSharedBuffers(DataSource ds, String instanceName) {
		...
		try (Connection conn = ds.getConnection(); Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(sql)) {
			if (rs.next()) {
				...
				if (sharedBuffersBytes < 128 * 1024 * 1024) {
					...
					ur.setDescription("shared_buffers is set to " + sharedBuffers + ", which may be too low for production workloads");
					ur.setRationale("Recommended to set shared_buffers to 25% of available RAM, " + "typically at least 1GB for production databases");
