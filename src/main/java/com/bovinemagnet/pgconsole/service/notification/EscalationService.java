...
public class EscalationService {
	...
	public List<NotificationResult> escalateAlert(ActiveAlert alert) {
		...
		EscalationPolicy policy = policyRepository.findById(alert.getEscalationPolicyId()).orElse(null);
		...
		LOG.infof("Escalating alert %s from tier %d to tier %d", alert.getAlertId(), currentTier, nextTier.getTierOrder());
	...
	public static class EscalationStatus {
		...
		public EscalationStatus(ActiveAlert alert, EscalationPolicy policy) {
			...
			if (alert.getLastNotificationAt() != null) {
				this.minutesSinceLastNotification = java.time.Duration.between(alert.getLastNotificationAt(), Instant.now()).toMinutes();
			} else {
			...
			if (policy != null && currentTier < maxTier) {
				...
				if (nextTier != null) {
					this.minutesUntilNextEscalation = Math.max(0, nextTier.getDelayMinutes() - (int) minutesSinceLastNotification);
				} else {
		...
		public long getMinutesSinceLastNotification() {
			return minutesSinceLastNotification;
		}

		public EscalationTier getNextTier() {
			return nextTier;
		}

		public int getMinutesUntilNextEscalation() {
			return minutesUntilNextEscalation;
		}
