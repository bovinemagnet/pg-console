...
pg-console samples database metrics at regular intervals to provide historical analysis and sparkline graphs.
...
|`true`
|Enable or disable historical metric sampling
...
To disable history sampling (e.g., for read-only monitoring):
...
* Flyway schema migrations to `pgconsole` schema
